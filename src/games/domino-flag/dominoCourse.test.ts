...
describe('createDominoCourse', () => {
  ...
  it('long縺ｮ繧ｫ繝｡繝ｩ騾ｲ陦悟ｺｦ縺ｮ蜈磯ｭ縺ｯ道中44枚の後に共有直線12枚が続く', () => {
    ...
    expect(course.cameraProgressCount).toBe(course.approachCount + 12)
    expect(
      course.placements
        .slice(course.approachCount, course.cameraProgressCount)
  ...
  it('道中の枚数、id、終端の接続点を満たす', () => {
    ...
    expect(approach.length).toBeGreaterThanOrEqual(30)
    expect(approach.length).toBeLessThanOrEqual(45)
    expect(new Set(ids).size).toBe(approach.length)
    expect(ids).toEqual(approach.map((_, index) => `approach-${index}`))
  ...
  it('道中の旋回、自己干渉、通常コース干渉、地面内を満たす', () => {
    ...
    const yawChanges = approach.slice(1).map((placement, index) => {
      const previous = approach[index]!
      return placement.yaw! - previous.yaw!
    })
    const adjacentDistances = approach.slice(1).map((placement, index) =>
      distance(placement, approach[index]!),
    )
