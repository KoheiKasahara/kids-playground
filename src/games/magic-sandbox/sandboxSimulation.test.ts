...
describe('falling sand play loop', () => {
  ...
  it('tulip seeds grow just like flower seeds but open into a red cup', () => {
    const flower = bed(Cell.Mud)
    const tulip = new Sandbox(48, 48, seeded())
    tulip.cells.fill(Cell.Mud, 36 * 48)
    tulip.paint({ x: 24, y: 34 }, Cell.TulipSeed, 0)
    advance(flower, 130); advance(tulip, 130)
    expect(tulip.flowers).toBe(1)
    expect(tulip.bloomingFlowers()).toEqual(flower.bloomingFlowers())
    expect(tulip.seedOf(tulip.bloomingFlowers()[0])).toBe(Cell.TulipSeed)
    expect(flower.seedOf(flower.bloomingFlowers()[0])).toBe(Cell.Seed)
    expect(count(tulip, Cell.TulipPetal)).toBeGreaterThan(0)
    expect(count(tulip, Cell.Petal)).toBe(0)
    expect(count(flower, Cell.TulipPetal)).toBe(0)
    expect(count(tulip, Cell.TulipSeed)).toBe(0)
    const dry = new Sandbox(48, 48, seeded())
    dry.cells.fill(Cell.Sand, 36 * 48)
    dry.paint({ x: 24, y: 4 }, Cell.TulipSeed, 0)
    advance(dry, 150)
    expect(dry.flowers).toBe(0)
    expect(dry.get(24, 35)).toBe(Cell.TulipSeed)
  })
