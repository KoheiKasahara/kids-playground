...
export default function WorldTravelQuizPlay() {
  ...
  if (!isTravelRegion(pathRegion)) return <Navigate to="/games/world-travel-quiz" replace />
...
function TravelGame({ region }: { region: TravelRegion }) {
  ...
  const [questions] = useState(() => generateTravelQuestions(course))
  ...
  const next = () => {
    ...
    if (isLast) { navigate(`/games/world-travel-quiz/${region}/result`, { replace: true, state: { correctCount, totalCount: QUESTION_COUNT, courseId: course.id } }); return }
    setPhase('traveling')
  }
