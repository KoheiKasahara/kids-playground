...
import { isTravelRegion, type TravelCourse } from './types'
...
export default function WorldTravelQuizResult() {
  const { region: pathRegion } = useParams(); const location = useLocation(); const navigate = useNavigate()
  if (!isTravelRegion(pathRegion) || !isResult(location.state)) return <Navigate to="/games/world-travel-quiz" replace />
  ...
  return <Result course={course} score={location.state.correctCount} onAgain={() => navigate(`/games/world-travel-quiz/${pathRegion}/play`, { replace: true })} onHome={() => navigate('/games/world-travel-quiz')} onRoot={() => navigate('/')} />
}
