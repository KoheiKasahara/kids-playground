...
test('新しいしかけを選んで置け、せんぷうきを回せる', async () => {
  await renderGame()
  fireEvent.click(trayPart('せんぷうき'))
  expect(screen.getByRole('status')).toHaveTextContent('かぜで おすよ')
  tapBoard(1, 2)
  expect(engineMock.options?.parts[0].typeId).toBe('fanRight')
  tapBoard(1, 2)
  fireEvent.click(screen.getByRole('button', { name: 'まわす' }))
  expect(engineMock.options?.parts[0].typeId).toBe('fanLeft')
  fireEvent.click(trayPart('あわリフト'))
  tapBoard(3, 4)
  fireEvent.click(trayPart('ワープ いりぐち'))
  tapBoard(5, 5)
  fireEvent.click(trayPart('ワープ でぐち'))
  tapBoard(1, 7)
  expect(engineMock.options?.parts.map(p => p.typeId)).toEqual(['fanLeft', 'bubbleLift', 'warpIn', 'warpOut'])
  tapBoard(3, 7)
  expect(engineMock.options?.parts).toHaveLength(4)
})
