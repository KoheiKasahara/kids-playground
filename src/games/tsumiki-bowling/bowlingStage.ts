 ...
 * たからものの小さな連鎖へ届く。ここを広くすると（実測0.98のとき）中くらいの
 * 強さでまん中へ投げた玉が2つの門を素通りし、たからもの1個しか倒れなかった。
 */
const GATE_STAGE: BowlingStage = {
  ...
  blocks: [
    ...
    // 奥のたからもの。すき間をすり抜けたちいさいだまが最初の宝箱へ当たり、
    // 奥へ並べた小さな積み木へ「ぱたぱた」と連鎖する。
    // どの箱も門の柱(内側面±0.34)と横に重ならない大きさにしてある
    // （初期状態で重なると、Rapierが弾き飛ばして勝手に崩れる）。
    { x: 0, z: -9.1, height: 0.25, size: [0.6, 0.5, 0.6], color: YELLOW },
    { x: 0, z: -9.75, height: 0.19, size: [0.48, 0.38, 0.48], color: ORANGE },
  ],
}
