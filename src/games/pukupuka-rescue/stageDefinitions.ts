...
export const PUKUPUKA_STAGE: StageDefinition = {
  ...
  // 水車（#520）。ゲートより手前（左側）の壁ぎわに取り付ける。水面が届かないあいだは
  // ただの飾りだが、じゃぐちで水位を上げて羽根の下半分(cy+radius=83)より上まで満たすと
  // 浸かって回りだし、触れている浮遊物をゴールの向きへ弱く押し流す（連動アクション）。
  // タップ操作は持たせず、水の状態だけで「回る/止まる」が決まるようにしてある。
  waterWheel: { id: 'main-water-wheel', cx: 23, cy: 74, radius: 9 },
