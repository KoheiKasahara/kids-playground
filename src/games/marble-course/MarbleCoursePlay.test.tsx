...
it('erases only the chosen part, and asks again before wiping the whole board', () => {
  show()
  click('まっすぐを ついか')
  click('ゴールを ついか')
  expect(engine.options!.course.parts).toHaveLength(3)
  click('けす')
  expect(engine.options!.course.parts.map(part => part.id)).toEqual(['part-0', 'new-0'])
  expect(engine.options!.selectedId).toBe('new-0')
  expect(screen.getByRole('status')).toHaveTextContent('けしたよ')
  // One tap only asks; anything else in between calls the question off.
  click('ぜんぶけす')
  expect(engine.options!.course.parts).toHaveLength(2)
  expect(screen.getByRole('status')).toHaveTextContent('ぜんぶ けす？')
  click('まっすぐを ついか')
  expect(screen.getByRole('button', { name: 'ぜんぶけす' })).toBeEnabled()
  click('ぜんぶけす')
  click('ほんとに？')
  expect(engine.options!.course.parts).toHaveLength(0)
