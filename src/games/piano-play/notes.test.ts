...
describe('piano notes', () => {
  test('C4〜E5を自然な半音順で定義する', () => {
    expect(PIANO_NOTES.map((note) => note.id)).toEqual([
      'C4', 'C#4', 'D4', 'D#4', 'E4', 'F4', 'F#4', 'G4', 'G#4', 'A4', 'A#4', 'B4', 'C5', 'C#5', 'D5', 'D#5', 'E5',
    ])
    expect(WHITE_NOTES.map((note) => note.id)).toEqual(['C4', 'D4', 'E4', 'F4', 'G4', 'A4', 'B4', 'C5', 'D5', 'E5'])
    expect(WHITE_NOTES).toHaveLength(10)
    expect(BLACK_NOTES).toHaveLength(7)
    expect(BLACK_NOTES.map((note) => note.whiteKeyIndex)).toEqual([0, 1, 3, 4, 5, 7, 8])
  })
  ...
  test('A4は440Hzで、IDからノートを参照できる', () => {
    ...
    expect(findPianoNote('D5')?.frequency).toBeCloseTo(587.330, 3)
    expect(findPianoNote('E5')?.frequency).toBeCloseTo(659.255, 3)
  })
})
