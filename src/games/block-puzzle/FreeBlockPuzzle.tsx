...
/**
 * ブロックパズルの「じゆうに ならべる」モード（#480 Phase 1 + #481 Phase 2 + #482 Phase 3 +
 * #483 Phase 4）。#711 でおちてくるモードが加わったため、モードえらびから入る2つのうちの
 * 片方になったが、遊び自体は今までどおり変えていない。
 *
 ...
 * ドラッグでの移動・入れ替えを追加した。
 *
 * このモードは落下・ライン消去・時間制限・スコア・ゲームオーバー・ランダム配布を持たない
 * （落ちてくる遊びは FallingBlockPuzzle が独立して受け持つ）。操作が不正なとき
 * （盤面外／重なり）も失敗にはせず、短いことばと赤いわくで知らせるだけにしている。
 *
 ...
 */
export default function FreeBlockPuzzle({ onBack }: Props) {
    ...
    <main className={styles.page}>
      <header className={styles.header}>
        ...
        <h1 className={styles.title}>
          <span aria-hidden="true">🧩</span> じゆうに ならべる
        </h1>
