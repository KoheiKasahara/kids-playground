...
/** モードえらび（BlockPuzzlePlay）を通さず、じゆうに ならべる モードだけを直接描く。 */
function renderPlay(onBack: () => void = () => {}) {
  return render(
    <MemoryRouter>
      <FreeBlockPuzzle onBack={onBack} />
    </MemoryRouter>,
  )
}
...
describe('ブロックパズル(じゆうに ならべる): 画面と操作', () => {
...
describe('ブロックパズル(じゆうに ならべる): まわす（未配置パーツ）', () => {
...
describe('ブロックパズル(じゆうに ならべる): 配置済みパーツの選択', () => {
...
describe('ブロックパズル(じゆうに ならべる): 配置済みパーツの回転', () => {
...
describe('ブロックパズル(じゆうに ならべる): 配置済みパーツの移動', () => {
...
describe('ブロックパズル(じゆうに ならべる): けす（削除）', () => {
...
describe('ブロックパズル(じゆうに ならべる): 完成判定・完成演出（#482）', () => {
...
describe('ブロックパズル(じゆうに ならべる): ドラッグでの移動・入れ替え（#483）', () => {
...
describe('ブロックパズル(じゆうに ならべる): スマホでの安定したドラッグ（#510）', () => {
...
describe('ブロックパズル(じゆうに ならべる): 新規パーツのドラッグ配置とプレビュー（#510）', () => {
...
describe('ブロックパズル(じゆうに ならべる): パーツ一覧から直接ドラッグ', () => {
