...
/**
 * 短辺に対してボールの直径が占める割合。
 * 幅390pxのスマホなら直径約74px、そこへ貼る国旗パネル（直径の62%）が約46pxになる。
 * FlagBallの円形クロップが「模様を見分けられる大きさ」として採用している48pxを
 * わずかに下回るが、パネルは円形に切らず4:3のまま枠線付きで見せるため、
 * 実機では国旗を判別できる。ここを上げるとボールが大きくなるかわりに見える範囲が狭まる。
 */
export const BALL_SCREEN_DIAMETER_RATIO = 0.19

/**
 * 寄りすぎて進行方向の通路が見えなくならないよう、短辺へ残す最小マス数。
 * 上のボール占有率だけで約3.51マスが入るため、通常はこちらの下限は効かない。
