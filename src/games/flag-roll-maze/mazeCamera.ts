 ...
 *
 * 幅390pxのスマホで直径約55px、そこへ貼る国旗パネル（直径の62%）が約34pxになる。
 * パネルは円形に切らず4:3のまま枠線付きで見せるので国旗として読める大きさを保ちつつ、
 * 実機確認で「スマホだとボールが近すぎる」という結果から、以前の0.173より引いている。
 * 寄りたいときはプレイ中の「＋」で以前の大きさ以上まで寄せられる。
 */
export const BALL_SCREEN_DIAMETER_RATIO = 0.14

/**
 * 寄りすぎて進行方向の通路が見えなくならないよう、短辺へ残す最小マス数。
 * 上のボール占有率だけで約4.8マスが入るため、通常はこちらの下限は効かない。
 ...
 * 全体は見渡せず、一番寄せてもボールは画面短辺の約20%に留まる。1段あたり
