...
import { createMazeStage, findMazePath, MAZE_STAGE_ROWS } from './mazeStage'
...
function blockedCellRects(): CellRect[] {
...
function wallCellRects(): CellRect[] {
  const rects: CellRect[] = []
  for (const [row, line] of MAZE_STAGE_ROWS.entries()) {
    for (let column = 0; column < line.length; column += 1) {
...
function holeCellRects(): CellRect[] {
  const rects: CellRect[] = []
  for (const [row, line] of MAZE_STAGE_ROWS.entries()) {
    for (let column = 0; column < line.length; column += 1) {
...
function openCell(column: number, row: number): boolean {
  const cell = MAZE_STAGE_ROWS[row]?.[column]
...
describe('既定ギミックの配置安全性', () => {
  ...
  it('回転棒の掃引円へ一度も入らずにSTARTからGOALまで到達できる', () => {
    ...
    const walls = wallCellRects()
    const holes = holeCellRects()
    const passable = samples.map((sample) => isPassableSample(sample, walls, holes))
  ...
  it('縦通路には穴のない連続したレーンが1本あり、ボール直径以上の幅を持つ', () => {
    ...
    const safeColumns = Array.from({ length: stage.columnCount }, (_, column) => column).filter(
      (column) => corridorRows.every((row) => openCell(column, row)),
    )
  ...
  it('バンパーの外周から部屋の両側の壁までボール直径以上の隙間が残る', () => {
    const cellSize = stage.boardWidth / stage.columnCount
    const leftWall = cellToWorld(0, 8, stage.columnCount, stage.rowCount).x + cellSize / 2
    const rightWall = cellToWorld(stage.columnCount - 1, 8, stage.columnCount, stage.rowCount).x - cellSize / 2
    for (const bumper of stage.gimmicks.bumpers) {
      expect(bumper.center.x - bumper.radius - leftWall).toBeGreaterThanOrEqual(BALL_RADIUS * 2 - EPSILON)
      expect(rightWall - bumper.center.x - bumper.radius).toBeGreaterThanOrEqual(BALL_RADIUS * 2 - EPSILON)
    }
  })
  ...
  it('チェックポイントが壁・穴・ギミックからボール半径以上離れている', () => {
    const blocked = blockedCellRects()
