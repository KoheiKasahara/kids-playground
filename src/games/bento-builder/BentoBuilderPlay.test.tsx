...
test('やりなおしですべてのおかずと選択を消す', () => {
  open()
  fireEvent.click(screen.getByRole('button', { name: 'つくる！' }))
  fireEvent.click(screen.getByRole('button', { name: 'にんじんを いれる' }))
  fireEvent.click(screen.getByRole('button', { name: 'ミニトマトを いれる' }))
  fireEvent.click(screen.getByRole('button', { name: '↺ やりなおし' }))
  expect(mock.latest!.foods).toEqual([])
  expect(mock.latest!.selected).toBeNull()
  expect(screen.getByRole('status')).toHaveTextContent('さいしょから つくろう！')
  expect(screen.getByRole('button', { name: '↺ やりなおし' })).toBeDisabled()
})
