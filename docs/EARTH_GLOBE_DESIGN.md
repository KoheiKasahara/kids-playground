...
簡略化データ      768ポリゴン / 37,950座標 / 最大リング1,707点 →  1,065ms  (約7.9倍)
```
...
- **簡略化のあとで、大円角が4度を超える辺には中間点を戻す。** Douglas-Peuckerは経緯度平面上のずれで点を間引くため、北緯49度に沿う米加国境のような「経緯度平面では完全な直線」の国境は中間点がすべて消える。しかし球面上では緯線は大円ではないので、残った2点を結ぶ辺は球の内側へ沈み込み、陸ポリゴンが海の球体の下へ潜って海の色が透ける。実際に米加国境が大円角18.03度の1本の辺になり、沈み込み1.235が陸の高さ0.8を超えて破綻していた。4度なら沈み込みは約0.06で十分な余裕がある。この再分割もarc単位で行うため国境は共有されたままになる。
