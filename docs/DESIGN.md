...
/games/flag-quiz/flag-to-name/play
└─ 国旗クイズプレイ（こっき → なまえ）

/games/flag-quiz/flag-to-name/result
└─ 国旗クイズ結果（こっき → なまえ）

/games/flag-quiz/name-to-flag/play
└─ 国旗クイズプレイ（なまえ → こっき）

/games/flag-quiz/name-to-flag/result
└─ 国旗クイズ結果（なまえ → こっき）
```

モード追加前の旧URL（`/games/flag-quiz/play`, `/games/flag-quiz/result`）は、
ブックマークやPWAホーム画面ショートカットの互換のためリダイレクトとして残す。
`/play` は旧仕様どおり「こっき → なまえ」へ、`/result` は結果がstate（正解数など）に
依存し復元できないためモード選択画面へ遷移させる。
...
「国旗 → 国名」「国名 → 国旗」「4択」は実装済み。今後は以下を拡張予定。
