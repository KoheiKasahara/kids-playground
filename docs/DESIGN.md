...
初期リリースの「国旗クイズ」に加え、「はたらくくるまクイズ」を実装する。後者の画面・URL・24車両・素材・検収条件は [WORKING_VEHICLE_QUIZ_DESIGN.md](WORKING_VEHICLE_QUIZ_DESIGN.md) を正とする。新規ミニゲームの共通規約は [MINIGAME_DEVELOPMENT_GUIDELINES.md](MINIGAME_DEVELOPMENT_GUIDELINES.md) に定める。
