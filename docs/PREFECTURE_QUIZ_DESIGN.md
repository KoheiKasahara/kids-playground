...
- 小さい都府県は地方ごとの表示範囲を詰め、まず見た目そのものを大きくします。なお関東・近畿・九州の狭い県は、主図上の番号と対応する下部の見える補助タップ枠も出します。補助枠は県pathと重ならない専用レールに置き、回答後も同位置で正誤記号・県名を表示します。320px実測で東京・大阪・香川を含む全県を確認します。
...
- 320×568と375×667の縦画面で、問題、回答、次へ、やめるが画面外へ出ない。
- 東京、大阪、香川、佐賀を含む全地方で、見える県をタップでき、透明ヒット領域が隣県へ重ならない。
