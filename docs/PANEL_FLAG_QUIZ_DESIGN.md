...
- 画面下部の固定領域（`feedbackBar`）に、正誤メッセージ（「🎉 せいかい！」/「ざんねん！」）、正解の国名（「こたえ: {国名}」）、得点の詳細（正解時は「{N}まいで わかった！ {M}てん」、不正解時は「0てん」）、次の操作ボタンを表示する。
...
- 320px級の狭い縦画面（`@media (orientation: portrait) and (max-height: 620px)`）ではパネル枠・選択肢・フィードバックバーの高さとフォントサイズを縮小する。低い横画面（`@media (orientation: landscape) and (max-height: 560px)`）では、既存`FlagQuizPlay`のflagToNameモードと同じく本体を左（国旗）・右（設問・めくるボタン・選択肢）の2カラムに切り替える。
- 国旗エリア（`PanelFlag`の`.frame`）は既存`FlagImage`の`.large`と同じ考え方（`height: 100%`＋`aspect-ratio: 4/3`＋`max-height`）で伸縮するため、パネルグリッドは常に国旗画像とぴったり重なる。
