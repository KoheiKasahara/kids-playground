...
test('the city map has two points, and the first one sends the train underground', async ({ page }) => {
  test.setTimeout(120_000)
  const errors = capturePageErrors(page)
  await page.goto('/games/train-journey')
  await page.getByRole('button', { name: 'とかいの コースを えらぶ', exact: true }).click()
  const start = page.getByRole('button', { name: 'しゅっぱつ！', exact: true })
  await expect(start).toBeEnabled({ timeout: 20_000 })
  await start.click()
  await expect(page.locator('canvas')).toHaveCount(1)
  const scene = page.getByTestId('journey-scene')
  await page.getByRole('button', { name: 'ひとつめの ポイントを きりかえる', exact: true }).click()
  await expect(page.getByRole('complementary')).toHaveAccessibleName('コースマップ。つぎは ちかてつ')
  await page.getByRole('button', { name: 'ふたつめの ポイントを きりかえる', exact: true }).click()
  await expect(page.getByRole('button', { name: 'ふたつめの ポイントを きりかえる', exact: true })).toContainText('みなと')
  await page.getByRole('button', { name: 'かそく！', exact: true }).click()
  await expect.poll(() => scene.getAttribute('data-edge'), { timeout: 90_000 }).toBe('subway')
  await expect(page.getByRole('button', { name: 'ふたつめの ポイントを きりかえる', exact: true })).toHaveAttribute('data-next', 'true')
  await page.screenshot({ path: 'test-results/train-journey-downtown.png' })
  expect(Number(await scene.getAttribute('data-draw-calls'))).toBeLessThan(65)
  expect(Number(await scene.getAttribute('data-triangles'))).toBeLessThan(150_000)
  expect(errors).toEqual([])
})
