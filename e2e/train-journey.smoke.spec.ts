...
test('the town branch carries the train through its level crossing', async ({ page }) => {
  test.setTimeout(90_000)
  const errors = capturePageErrors(page)
  await depart(page)
  const scene = page.getByTestId('journey-scene')
  for (let i = 0; i < 2; i++) await page.getByRole('button', { name: 'ポイントを きりかえる', exact: true }).click()
  await expect(page.getByRole('complementary')).toHaveAccessibleName('コースマップ。つぎは まち')
  await page.getByRole('button', { name: 'かそく！', exact: true }).click()
  await expect.poll(() => scene.getAttribute('data-edge'), { timeout: 60_000 }).toBe('city')
  await expect(page.getByText(/^(ビルの まちを はしるよ！|ふみきり カンカン！)$/)).toBeVisible()
  await page.screenshot({ path: 'test-results/train-journey-city.png' })
  expect(Number(await scene.getAttribute('data-draw-calls'))).toBeLessThan(65)
  expect(Number(await scene.getAttribute('data-triangles'))).toBeLessThan(150_000)
  expect(errors).toEqual([])
})
