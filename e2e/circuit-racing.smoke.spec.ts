...
test('車を選んで走行・停止・カメラ変更・選び直しができる', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'やすむ', exact: true }).click()
  // Allow the frame carrying the pause state to render, then verify it remains still.
...
test('全コースの描画を切り替え、WebGL復帰後も走れる', async ({ page }) => {
  ...
  await page.goto('/games/circuit-racing')
  await page.getByRole('button', { name: '3だい', exact: true }).click()
  for (const course of ['くねくねカーブ', 'びゅんびゅんオーバル', 'ぐるっとヘアピン', 'みんなのサーキット']) {
